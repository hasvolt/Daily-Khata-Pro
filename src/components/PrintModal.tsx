...
<body>
  <div class="brand-header">
    <div>
      ...
      <div class="brand-sub">Smart Fund Systematic Money Ledger &amp; Wealth Audit</div>
    </div>
  ...
  ${(userDetails.name || userDetails.contact || userDetails.note) ? `
  <div class="user-details">
    ${userDetails.name ? `<div><strong style="color: #0284c7; font-size: 12px;">${escapeHtml(userDetails.name)}</strong></div>` : ''}
    ${userDetails.contact ? `<div>${escapeHtml(userDetails.contact)}</div>` : ''}
    ${userDetails.note ? `<div style="color: #475569; margin-top: 2px;">${escapeHtml(userDetails.note)}</div>` : ''}
  </div>
  ` : ''}
      ...
      <div className="bg-[var(--theme-surface,#0E1A29)] border border-[var(--theme-border,#213E61)] rounded-2xl w-full max-w-2xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden text-left">
        ...
        <div className="p-5 sm:p-6 overflow-y-auto space-y-5 bg-[var(--theme-bg,#070E18)] text-[#F8FAFC] text-[13px] leading-relaxed flex-1 max-h-[calc(92vh-180px)]">
          ...
          <div className="bg-[var(--theme-surface,#0E1A29)] border border-[var(--theme-border,#213E61)] rounded-xl p-4 no-print">
            <h3 className="text-[12px] font-bold text-[#F8FAFC] mb-3 uppercase tracking-wider">Add Details to Print Report (Optional)</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <input 
                type="text" 
                placeholder="Address or Note (Optional)"
                className="w-full bg-[var(--theme-bg,#070E18)] border border-[var(--theme-border,#213E61)] rounded-lg px-3 py-2 text-[12px] text-[#F8FAFC] focus:border-[#38BDF8] outline-none sm:col-span-2"
