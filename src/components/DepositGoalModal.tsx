        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-5 space-y-4 text-left">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[16px] font-bold text-[var(--theme-text-dim,#94A3B8)]">{getCurrencyConfig(getCurrentLanguage()).symbol}</span>
              <input
                type="number"
                step="any"
                min="1"
                ...
                autoFocus
                className="w-full bg-[var(--theme-bg,#070E18)] border border-[var(--theme-border,#213E61)] focus:border-[var(--theme-primary,#38BDF8)] text-[var(--theme-text,#F8FAFC)] rounded-xl pl-9 pr-4 py-3 text-[18px] font-mono font-bold focus:outline-none"
              />
            </div>
            ...
            <div className="flex gap-1.5 flex-wrap pt-1">
              {quickAmounts.map((q) => (
                <button
                  ...
                  className="px-2.5 py-1 text-[11px] font-mono font-bold rounded-lg bg-[var(--theme-surface,#0E1A29)] border border-[var(--theme-border,#213E61)] text-[var(--theme-text-dim,#94A3B8)] hover:text-[var(--theme-primary,#38BDF8)] hover:border-[var(--theme-primary,#38BDF8)]/50 transition-colors cursor-pointer"
                >
                  +{formatCurrency(q)}
                </button>
              ))}
          ...
          <div className="bg-[var(--theme-bg,#070E18)] border border-[var(--theme-border,#213E61)] rounded-xl p-3 space-y-2.5">
            ...
            {shouldDeduct && (
              <div className="pt-1.5 space-y-1.5 border-t border-[var(--theme-border,#213E61)]">
                <label className="text-[10.5px] font-bold text-[var(--theme-text-dim,#94A3B8)] uppercase">
                  Deduct from Fund:
                </label>
                <select
                  value={selectedFund}
                  onChange={(e) => setSelectedFund(e.target.value as FundType)}
                  className="w-full bg-[var(--theme-surface,#0E1A29)] border border-[var(--theme-border,#213E61)] focus:border-[var(--theme-primary,#38BDF8)] text-[var(--theme-text,#F8FAFC)] rounded-lg px-3 py-2 text-[12.5px] cursor-pointer focus:outline-none"
                >
                  {FUND_ORDER.map((f) => (
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full bg-[var(--theme-bg,#070E18)] border border-[var(--theme-border,#213E61)] focus:border-[var(--theme-primary,#38BDF8)] text-[var(--theme-text,#F8FAFC)] rounded-xl px-3.5 py-2 text-[13px] focus:outline-none"
            />
          </div>
