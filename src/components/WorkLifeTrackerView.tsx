...
export const WorkLifeTrackerView: React.FC<WorkLifeTrackerViewProps> = ({
  ...
  const getStatusBadge = (status: WorkLog['status']) => {
    ...
    switch (status) {
      ...
      case 'on_hold':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-[11px] font-bold bg-[#94A3B8]/15 text-[var(--theme-text-dim,#94A3B8)] border border-[#94A3B8]/30">
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-5 animate-in fade-in duration-200 text-left pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-[var(--theme-surface,#0E1A29)] border border-[var(--theme-border,#213E61)] rounded-2xl p-4 sm:p-5 shadow-md">
        <div>
          <h2 className="text-[20px] sm:text-[22px] font-bold text-[var(--theme-text,#F8FAFC)] flex items-center gap-2">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 mt-3 sm:mt-0">
          ...
          <div className="relative">
            <button
              ...
              className="w-10 h-10 flex items-center justify-center rounded-xl bg-[var(--theme-card,#132438)] border border-[var(--theme-border,#213E61)] text-[var(--theme-text,#F8FAFC)] hover:bg-[var(--theme-card-hover,#19304A)] transition-all cursor-pointer shadow-sm active:scale-95 shrink-0"
            ...
            {isExportMenuOpen && (
              <>
                ...
                <div className="absolute right-0 mt-2 w-48 bg-[var(--theme-surface,#0E1A29)] border border-[var(--theme-border,#213E61)] rounded-xl shadow-2xl z-50 p-1.5 animate-in fade-in zoom-in-95">
                  <div className="px-2.5 py-1.5 text-[10px] font-extrabold uppercase text-[var(--theme-text-dim,#94A3B8)] border-b border-[var(--theme-border,#213E61)]/60 mb-1">
                    Backup / Export
                  </div>
                  <button
                    type="button"
                    ...
                    className="w-full flex items-center gap-2 px-2.5 py-2 rounded-lg text-[12px] font-bold text-[var(--theme-text,#F8FAFC)] hover:bg-white/5 transition-colors cursor-pointer text-left"
                  >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-3.5 rounded-xl bg-[var(--theme-card,#132438)] border border-[var(--theme-border,#213E61)] shadow-sm">
          <span className="text-[11px] font-bold text-[var(--theme-text-dim,#94A3B8)] uppercase tracking-wider block">
            {getTrackerText('totalWorkItems', language)}
          </span>
          <div className="font-serif-display text-[22px] font-bold text-[var(--theme-text,#F8FAFC)] mt-0.5 font-mono">
            {workLogs.length}
          </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2.5">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-[var(--theme-card,#132438)] border border-[var(--theme-border,#213E61)] rounded-xl pl-10 pr-4 py-2.5 text-[13px] text-[var(--theme-text,#F8FAFC)] placeholder:text-[#64748B] focus:outline-none focus:border-[var(--theme-primary,#38BDF8)]"
          />
        </div>
        ...
        {activeTab === 'work' && (
          <div className="flex gap-2">
            <select
              ...
              className="bg-[var(--theme-card,#132438)] border border-[var(--theme-border,#213E61)] text-[var(--theme-text-muted,#CBD5E1)] text-[12.5px] font-bold rounded-xl px-3 py-2.5 focus:outline-none"
            >
            ...
            {uniqueWorkCategories.length > 0 && (
              <select
                ...
                className="bg-[var(--theme-card,#132438)] border border-[var(--theme-border,#213E61)] text-[var(--theme-text-muted,#CBD5E1)] text-[12.5px] font-bold rounded-xl px-3 py-2.5 focus:outline-none max-w-[150px] truncate"
              >
        ...
        <div className="space-y-3">
          {filteredWorkLogs.length === 0 ? (
            <div className="bg-[var(--theme-card,#132438)] border border-[var(--theme-border,#213E61)] rounded-2xl p-8 sm:p-12 text-center space-y-3">
              ...
              <p className="text-[13px] text-[var(--theme-text-dim,#94A3B8)] max-w-sm mx-auto">
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2.5">
                    <div className="flex items-start gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <h4 className="font-bold text-[15px] sm:text-[16px] text-[var(--theme-text,#F8FAFC)] group-hover:text-[var(--theme-primary,#38BDF8)] transition-colors">
                            {item.title}
                          </h4>
                          {getStatusBadge(item.status)}
                        </div>

                        <div className="flex flex-wrap items-center gap-3 text-[12px] text-[var(--theme-text-dim,#94A3B8)] mt-1">
                          ...
                          {item.clientOrCompany && (
                            <span className="flex items-center gap-1 font-semibold text-[var(--theme-text-muted,#CBD5E1)]">
                              <Building className="w-3.5 h-3.5 text-[#64748B]" />
                              <span>{item.clientOrCompany}</span>
                            </span>
                          )}

                          <span className="px-2 py-0.5 rounded bg-[var(--theme-bg,#070E18)] border border-[var(--theme-border,#213E61)] text-[11px] font-bold text-[var(--theme-text-muted,#CBD5E1)]">
                            {item.category}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="flex sm:flex-col items-end justify-between sm:justify-start gap-1 shrink-0 pt-1 sm:pt-0 border-t sm:border-t-0 border-[var(--theme-border,#213E61)]/50">
                      ...
                      {item.hoursSpent ? (
                        <div className="text-[12px] font-bold text-[var(--theme-text-dim,#94A3B8)] flex items-center gap-1">
                  ...
                  {/* Notes / Deliverables */}
                  {item.notes && (
                    <p className="text-[12.5px] text-[var(--theme-text-muted,#CBD5E1)] bg-[var(--theme-bg,#070E18)]/80 p-2.5 rounded-xl border border-[var(--theme-border,#213E61)]/60">
                      {item.notes}
                    </p>
                  )}
                  ...
                  {item.deliverables && item.deliverables.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {item.deliverables.map((d, i) => (
                        <span
                          key={i}
                          className="px-2 py-0.5 rounded-md bg-[var(--theme-surface,#0E1A29)] text-[var(--theme-text-dim,#94A3B8)] text-[11px] font-medium border border-[var(--theme-border,#213E61)] flex items-center gap-1"
                        >
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-[var(--theme-border,#213E61)]/60 text-[12px]">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="p-1.5 text-[var(--theme-text-dim,#94A3B8)] hover:text-[var(--theme-primary,#38BDF8)] rounded-lg hover:bg-[var(--theme-surface,#0E1A29)] transition-colors cursor-pointer"
                        title="Edit Work Log"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 text-[var(--theme-text-dim,#94A3B8)] hover:text-[#EF4444] rounded-lg hover:bg-[var(--theme-surface,#0E1A29)] transition-colors cursor-pointer"
                ...
                >
                  <div className="flex items-start justify-between gap-2.5">
                    <div className="flex items-start gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <h4 className="font-bold text-[15px] sm:text-[16px] text-[var(--theme-text,#F8FAFC)]">
                        ...
                        <div className="flex items-center gap-3 text-[12px] text-[var(--theme-text-dim,#94A3B8)] mt-1">
                          <span className="flex items-center gap-1">
                            <Calendar className="w-3.5 h-3.5 text-[#64748B]" />
                            <span>{item.date}</span>
                          </span>
                          ...
                          {item.wakeTime && (
                            <span className="text-[11.5px] text-[var(--theme-text-muted,#CBD5E1)]">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        ...
                        className="p-1.5 text-[var(--theme-text-dim,#94A3B8)] hover:text-[var(--theme-secondary,#FFC700)] rounded-lg hover:bg-[var(--theme-surface,#0E1A29)] transition-colors cursor-pointer"
                        title="Edit Story"
                      >
                        <Edit2 className="w-3.5 h-3.5" />
                      </button>
                  ...
                  {/* Highlights */}
                  <div className="text-[13px] text-[var(--theme-text,#F8FAFC)] leading-relaxed bg-[var(--theme-bg,#070E18)]/80 p-3 rounded-xl border border-[var(--theme-border,#213E61)]/60">
                    {item.highlights}
                  </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1 text-[12px]">
                      {item.morningRoutine && (
                        <div className="p-2 rounded-xl bg-[var(--theme-surface,#0E1A29)] border border-[var(--theme-border,#213E61)]">
                          ...
                          <p className="text-[var(--theme-text-muted,#CBD5E1)] text-[11.5px]">{item.morningRoutine}</p>
                        </div>
                      )}
                      {item.afternoonRoutine && (
                        <div className="p-2 rounded-xl bg-[var(--theme-surface,#0E1A29)] border border-[var(--theme-border,#213E61)]">
                          ...
                          <p className="text-[var(--theme-text-muted,#CBD5E1)] text-[11.5px]">{item.afternoonRoutine}</p>
                        </div>
                      )}
                      {item.eveningRoutine && (
                        <div className="p-2 rounded-xl bg-[var(--theme-surface,#0E1A29)] border border-[var(--theme-border,#213E61)]">
                          ...
                          <p className="text-[var(--theme-text-muted,#CBD5E1)] text-[11.5px]">{item.eveningRoutine}</p>
                        </div>
                      )}
                    </div>
                  )}
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 pt-1 text-[12px]">
                      {item.keyLearnings && (
                        <div className="flex-1 p-2.5 rounded-xl bg-[var(--theme-primary,#38BDF8)]/10 border border-[var(--theme-primary,#38BDF8)]/30">
                          ...
                          <p className="text-[var(--theme-text-muted,#CBD5E1)] text-[12px]">{item.keyLearnings}</p>
                        </div>
                      )}
                      {item.gratitude && (
                        <div className="flex-1 p-2.5 rounded-xl bg-[var(--theme-secondary,#FFC700)]/10 border border-[var(--theme-secondary,#FFC700)]/30">
                          ...
                          <p className="text-[var(--theme-text-muted,#CBD5E1)] text-[12px]">{item.gratitude}</p>
                        </div>
                      )}
                    </div>
                  )}
