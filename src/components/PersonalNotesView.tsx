...
export const PersonalNotesView: React.FC<PersonalNotesViewProps> = ({
  ...
  const getColorClasses = (color?: string) => {
    switch (color) {
      ...
      case 'slate':
        return 'bg-[#0F172A] border-[#334155] text-[var(--theme-text,#F8FAFC)]';
      default:
        return 'bg-[var(--theme-card,#132438)] border-[var(--theme-border,#213E61)] text-[var(--theme-text,#F8FAFC)]';
    }
  };
    ...
    <div className="w-full space-y-6 animate-in fade-in duration-200 text-left max-w-5xl mx-auto pb-12">
      ...
      <div className="bg-[var(--theme-card,#132438)] border border-[var(--theme-border,#213E61)] rounded-2xl sm:rounded-3xl p-5 sm:p-7 shadow-xl relative overflow-hidden transition-colors duration-300">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1.5 max-w-2xl">
            ...
            <h2 className="font-serif-display text-[24px] sm:text-[30px] font-bold text-[var(--theme-text,#F8FAFC)] tracking-tight leading-tight">
              {getNotesText('headerTitle', language)}
            </h2>
            <p className="text-[13px] sm:text-[14px] text-[var(--theme-text-dim,#94A3B8)] leading-relaxed">
        ...
        <div className="grid grid-cols-3 gap-2.5 sm:gap-4 mt-5 pt-4 border-t border-[var(--theme-border,#213E61)]/70">
          <div className="p-3 rounded-xl bg-[var(--theme-bg,#070E18)]/80 border border-[var(--theme-border,#213E61)]/60 text-center sm:text-left">
            <div className="text-[10.5px] sm:text-[11.5px] font-bold uppercase tracking-wider text-[var(--theme-text-dim,#94A3B8)]">
              {getNotesText('totalNotes', language)}
            </div>
            <div className="text-[18px] sm:text-[22px] font-bold text-[var(--theme-text,#F8FAFC)] mt-0.5">
              {notes.length}
            </div>
          </div>
      ...
      <div className="bg-[var(--theme-surface,#0E1A29)] border border-[var(--theme-border,#213E61)] rounded-2xl p-3.5 sm:p-4 shadow-md transition-all">
        {!isQuickBoxExpanded ? (
          <div
            ...
            className="flex items-center justify-between px-3 py-2 rounded-xl bg-[var(--theme-bg,#070E18)] border border-[var(--theme-border,#213E61)] text-[var(--theme-text-dim,#94A3B8)] cursor-pointer hover:text-[var(--theme-text,#F8FAFC)] hover:border-[var(--theme-primary,#38BDF8)] transition-all text-[13px]"
          >
            <div className="flex items-center gap-2">
          ...
          <form onSubmit={handleQuickSubmit} className="space-y-3 animate-in fade-in duration-150">
            <input
              ...
              placeholder="Title..."
              className="w-full px-3.5 py-2 rounded-xl bg-[var(--theme-bg,#070E18)] border border-[var(--theme-border,#213E61)] text-[var(--theme-text,#F8FAFC)] placeholder-[#64748B] text-[13.5px] focus:outline-none focus:border-[var(--theme-primary,#38BDF8)]"
              autoFocus
            />
            <textarea
              value={quickContent}
              ...
              className="w-full px-3.5 py-2.5 rounded-xl bg-[var(--theme-bg,#070E18)] border border-[var(--theme-border,#213E61)] text-[var(--theme-text,#F8FAFC)] placeholder-[#64748B] text-[13px] leading-relaxed focus:outline-none focus:border-[var(--theme-primary,#38BDF8)] resize-none"
            />
            <div className="flex items-center justify-end gap-2">
              <button
                ...
                className="py-1.5 px-3 rounded-lg text-[12px] font-bold text-[var(--theme-text-dim,#94A3B8)] hover:text-[var(--theme-text,#F8FAFC)] cursor-pointer"
              >
      ...
      <div className="space-y-3">
        ...
        <div className="relative">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--theme-text-dim,#94A3B8)] pointer-events-none" />
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder={getNotesText('searchPlaceholder', language)}
            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-[var(--theme-card,#132438)] border border-[var(--theme-border,#213E61)] text-[var(--theme-text,#F8FAFC)] placeholder-[#64748B] text-[13.5px] focus:outline-none focus:border-[var(--theme-primary,#38BDF8)] transition-colors shadow-sm"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery('')}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--theme-text-dim,#94A3B8)] hover:text-[var(--theme-text,#F8FAFC)] text-[11.5px] px-1.5 py-0.5 rounded bg-[#070E18] cursor-pointer"
            >
              Clear
            </button>
          )}
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-[12px] font-bold shrink-0 transition-all border cursor-pointer ${
              activeCategory === 'all' && !activeTag
                ...
                : 'bg-[var(--theme-card,#132438)] border-[var(--theme-border,#213E61)] text-[var(--theme-text-dim,#94A3B8)] hover:text-[var(--theme-text,#F8FAFC)]'
            }`}
          >
      ...
      {filteredNotes.length === 0 ? (
        <div className="p-8 sm:p-12 text-center rounded-2xl sm:rounded-3xl bg-[var(--theme-card,#132438)] border border-[var(--theme-border,#213E61)] space-y-4 shadow-md">
          ...
          <div className="space-y-1.5 max-w-md mx-auto">
            <h3 className="text-[18px] font-bold text-[var(--theme-text,#F8FAFC)]">
            ...
            return (
              ...
              >
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        ...
                        className={`p-1.5 rounded-lg transition-colors cursor-pointer ${
                          note.isPinned
                            ...
                            : 'text-[#64748B] hover:text-[var(--theme-text,#F8FAFC)] hover:bg-[#070E18]/50'
                        }`}
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg text-[#64748B] hover:text-[var(--theme-text,#F8FAFC)] hover:bg-[#070E18]/50 transition-colors cursor-pointer"
                  ...
                  {/* Title */}
                  <h4 className="font-serif-display font-bold text-[17px] text-[var(--theme-text,#F8FAFC)] leading-snug tracking-tight">
                    {note.title}
                  </h4>
                  ...
                  <div className="relative min-h-[60px] pt-1">
                    {isMasked ? (
                      ...
                      <div className="text-[13px] text-[var(--theme-text-muted,#CBD5E1)] whitespace-pre-wrap leading-relaxed break-words font-sans max-h-48 overflow-y-auto custom-scrollbar">
                        {note.content}
                      </div>
                    )}
                  </div>
                ...
                <div className="pt-3 mt-3 border-t border-[var(--theme-border,#213E61)]/50 space-y-2">
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <div className="text-[11px] text-[var(--theme-text-dim,#94A3B8)] flex items-center gap-1">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        ...
                        className={`p-1.5 rounded-lg text-[11px] transition-colors cursor-pointer ${
                          note.isLocked
                            ...
                            : 'text-[#64748B] hover:text-[var(--theme-text,#F8FAFC)]'
                        }`}
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg text-[#64748B] hover:text-[var(--theme-text,#F8FAFC)] transition-colors cursor-pointer"
