...
export const InvoicePrintView: React.FC<InvoicePrintViewProps> = ({
  ...
  return (
    <div
      ...
      className="w-full max-w-[850px] mx-auto bg-white text-slate-900 shadow-2xl rounded-2xl p-6 sm:p-10 border border-slate-200 print:border-none print:shadow-none print:p-0 print:m-0 print:max-w-none print:rounded-none print:text-black font-sans leading-normal selection:bg-blue-100"
      style={{ minHeight: '1050px' }}
    >
      {/* Header Bar */}
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 pb-6 border-b-2 border-slate-900 print:border-black">
        <div className="flex items-center gap-3.5 min-w-0">
          {sender.logoUrl ? (
            <img
              src={sender.logoUrl}
              alt={sender.businessName}
              className="h-16 w-16 sm:h-20 sm:w-20 object-contain rounded-lg border border-slate-200 p-1 shrink-0"
            />
          ) : (
            <div className="h-14 w-14 sm:h-16 sm:w-16 rounded-xl bg-slate-900 text-white flex items-center justify-center font-black text-xl tracking-wider shrink-0 print:bg-black">
        ...
        <div className="sm:text-right w-full sm:w-auto shrink-0 flex flex-col sm:items-end">
          <div className="inline-flex items-center gap-2">
            ...
            <span
              className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider print:border print:border-black ${
                status === 'paid'
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-6 pt-4 border-t-2 border-slate-900 print:border-black">
        ...
        <div className="w-full sm:w-5/12 space-y-1.5 text-xs">
          ...
          {/* Grand Total */}
          <div className="flex justify-between items-center py-2.5 px-3 bg-slate-900 text-white rounded-xl font-bold text-sm sm:text-base print:bg-transparent print:text-black print:border-y-2 print:border-black mt-2">
      ...
      {/* Footer Branding Note & Certification */}
      <div className="pt-6 mt-6 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between text-[10px] text-slate-600 font-medium">
        <span className="font-semibold text-slate-800">
          Generated By Daily Khata pro • <span className="text-blue-700 font-mono font-bold">www.rozfiber.com</span>
        </span>
        <span className="text-slate-500 text-[9.5px] mt-1 sm:mt-0">
          Computer generated invoice • 100% Free & Private Finance Platform
        </span>
      </div>
    </div>
  );
