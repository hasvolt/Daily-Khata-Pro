...
if (rootMatch) {
  let rootContent = rootMatch[1]
    ...
    .replace(/--theme-card: #[0-9A-Fa-f]+;/g, '--theme-card: #121212;')
    .replace(/--theme-card-hover: #[0-9A-Fa-f]+;/g, '--theme-card-hover: #1C1C1C;')
    .replace(/--theme-border: #[0-9A-Fa-f]+;/g, '--theme-border: #2A2A2A;');
